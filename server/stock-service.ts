...
class StockDataService {
  ...
  async getAnalyzedStocks(symbols: string[] = INDIAN_SYMBOLS.slice(0, 50)): Promise<InsertStock[]> {
    ...
    for (const [symbol, quote] of quotesArray) {
      ...
      // Always include permanent favorites, even with weak signals
      if (analysis.hasValidSignal || isPermanentFavorite) {
        // For permanent favorites, ensure they have at least basic signal data
        if (isPermanentFavorite && !analysis.hasValidSignal) {
          analysis.timeframes = ["1h"]; // Give at least one timeframe
          analysis.hasValidSignal = true;
        }
        
        console.log(`Including stock: ${quote.symbol}, isPermanentFavorite: ${isPermanentFavorite}, hasValidSignal: ${analysis.hasValidSignal}`);
